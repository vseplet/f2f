...
peer_pub`** (cleartext, индексируется); отдельный `is_mine` флаг не нужен
...
`Author` = `peer_pub`, и пир = пользователь → «мои» = записи этого пира. Кросс-
девайсного «все мои устройства» нет: другое устройство — это другой пир со своим
логом (см. [ENTITIES.md](ENTITIES.md)).
