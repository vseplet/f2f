...
#### Резолв `.f2f` из Docker-контейнеров

Основной резолвер слушает только `127.0.0.1` (эфемерный порт), и ОС
направляется к нему по зоне через systemd-resolved / `/etc/resolver`.
Контейнеру это недоступно: его `127.0.0.1` — это он сам.

Поэтому engine поднимает **второй DNS-листенер на overlay-IP ноды с
фиксированным портом `53`** — `100.64.x.y:53` (в логе:
`serving <zone>.f2f on 100.64.x.y:53 (for containers)`; overlay-IP видно
в разделе `camp` как *tunnel ip*). На нём **свои** домены отвечают
overlay-IP хоста, а не `127.0.0.1`, — тогда контейнер попадает на
reverse-proxy хоста (он тоже слушает overlay-IP `:443`), а не в свой
loopback. Чужие (пировские) домены отдаются overlay-IP пира, как обычно;
маршрут туда идёт через хост (`ip_forward=1` + NAT `100.64/10`).

Указываешь контейнеру этот резолвер:

```sh
docker run --dns 100.64.x.y ...
# или в docker-compose:
#   dns: [100.64.x.y]
# или глобально в /etc/docker/daemon.json: {"dns": ["100.64.x.y"]}
```

**Осторожно:** этот листенер отвечает только на `.f2f`, на остальное
(`google.com` и т.п.) отдаёт NXDOMAIN. Поэтому:
- **Если ставишь его единственным `--dns`** — сломается резолв всего
  не-`.f2f`. Так делать нельзя (пока листенер не форвардит апстрим).
- **Правильно — split-DNS:** направить на `100.64.x.y:53` только
  `*.f2f`, а обычный DNS оставить. Например внутри контейнера через
  dnsmasq: `server=/f2f/100.64.x.y`, или systemd-resolved:
  `Domains=~f2f` + `DNS=100.64.x.y` на нужном линке.
...
Встроенный OpenID Connect-провайдер (`services/oidc`): self-hosted
приложения (Gitea, Affine, …) логинятся «через f2f» по **passkey (Touch
ID)**, без своих паролей. У каждого пира свой IdP на
`auth-<fp8>.<zone>.f2f` (issuer), он же отдаётся под `/oidc` на доменах
приложений. `sub` = pub пира (берётся из AmneziaWG-attested заголовка
`X-F2F-Peer`), токены RS256. Вкладка **OIDC** в сайдбаре: issuer/discovery
для вставки в приложение, регистрация клиентов (client_id/secret,
callback/logout URL), список passkey-пользователей. Полностью —
[`OIDC.md`](OIDC.md).
...
## Деплой на Linux VPS — сетевые грабли

Проверено на Alpine VPS (Vultr). Большинство «нода зелёная, но связи нет» —
это локальный фаервол или особенности дистрибутива, а не баг f2f.

### Системный фаервол режет overlay (самая частая засада)

В игре **два разных UDP-порта**, их легко перепутать:

- **`9000/udp` — underlay AWG.** Сюда из интернета приходят зашифрованные
  пакеты туннеля между реальными публичными IP. Нужен для hole-punch.
- **`2203/udp` — bus (QUIC).** Живёт **внутри** туннеля, на overlay-адресе
  `100.64.0.X` (интерфейс `f2f0`). По нему идёт весь control-plane: health-пинги,
  чат, файлы, DNS-over-bus, shell/vnc. Это уже расшифрованный трафик.

Если на VPS стоит фаервол с политикой DROP (ufw/iptables), и открыт только
`9000`, то **сам туннель поднимается, а bus глохнет**: AWG расшифровал пакет,
отдал на `f2f0` → пакет идёт в цепочку INPUT → правила под `2203`/`f2f0` нет →
ядро его выбрасывает перед bus-сокетом. У f2f есть **своя** nft-таблица
(`inet f2f`), и она bus-порт пропускает (в UI «open ports» он built-in), но
она **не отменяет** дроп в чужом фаерволе: в netfilter если хоть одна цепочка
дропнула — всё, чужой `accept` уже не спасёт.

**Симптомы** (всё одновременно):
- пир в списке зелёный, `ping <overlay-ip>` ходит, DF-пинг полного размера
  (`ping -M do -s 1280 <overlay-ip>`) тоже ходит — то есть AWG и MTU в порядке;
- в логе `bus: dial: context deadline exceeded` или `ping … read: deadline`;
- `tcpdump -ni f2f0 host <peer> ` показывает входящие 1280-байтные Initial'ы,
  а сокет молчит;
- `ss -ulnp | grep 2203` показывает, что listener поднят на правильном IP;
- никаких ошибок quic-go (дроп делает ядро, не quic-go).

**Фикс** — доверить весь overlay-интерфейс (на нём и так только пиры с
валидными ключами, AWG это проверил), персистентно:

```sh
ufw allow in on f2f0           # ufw — сохраняется само
# или iptables, не забыть сохранить (iptables-save / служба iptables):
iptables -I INPUT -i f2f0 -j ACCEPT
```

Открывать один `2203/udp` мало — DNS, прокси и прочие overlay-сервисы
останутся так же зарезаны. Домашние ноды без системного фаервола работают
сразу — там и ловить нечего, грабли только на VPS.

### Особенности Alpine (musl / OpenRC / busybox)

- **`/tmp` — tmpfs (в RAM).** При `go build`/`make dev` линковщик пишет туда и
  падает с `no space left on device`, если RAM мало. Лечится:
  `export TMPDIR=/var/tmp/gobuild && mkdir -p $TMPDIR`.
- **busybox-утилиты урезаны.** Нужны нормальные `ip`/`ping` (с `-M do`):
  `apk add iproute2 iputils`. Для клона репы — `apk add git openssh-client`.
- **TUN заводится самим f2f**, руками `utun`/`f2f0` настраивать не надо.
- **Дешёвые планы Vultr = CGNAT** (`100.64/10` на eth0, без публичного egress) —
  hole-punch не взлетит, нужен reserved/публичный IPv4 для underlay-эндпоинта.
...
| `$HOME/.f2f/<camp_id>/{oidc_rsa.pem,clients.json,passkeys.json}` | OIDC: RS256 signing key, client registry, passkey credentials (0600) | root |
