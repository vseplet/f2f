...
Не-`.f2f` запросы этот листенер **форвардит на реальный резолвер хоста**
(из `/etc/resolv.conf`, fallback `1.1.1.1`), поэтому его можно ставить
**единственным** `--dns` контейнера — обычный DNS не сломается:

```sh
docker run --dns 100.64.x.y ...
# docker-compose:
#   dns: [100.64.x.y]
# глобально в /etc/docker/daemon.json: {"dns": ["100.64.x.y"]}
```

В логе при старте видно апстрим:
`serving <zone>.f2f on 100.64.x.y:53 (for containers, upstream [127.0.0.53:53])`.

Проверка изнутри контейнера: `getent hosts gitea.<zone>.f2f` → overlay-IP
хоста; `getent hosts google.com` → форвард на апстрим работает.
