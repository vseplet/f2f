...
Credential'ы сейчас хранятся в `passkeys.json` (`pub → []credential`),
**локально**. Целевая модель — публичная часть кред живёт в **`block.profile`**
(scope `profiles`, см. [IDENTITY.md](IDENTITY.md)) и синкается всем членам кэмпа.
Тогда IdP любого пира проверяет ассерцию удалённого гостя по креде из лога —
синк профиля **и есть** репликация кред. **Проверять** ассерцию может любой IdP
(общая креда + общий `rp.id`); **подписывать** токен — только тот, что провёл
вход (его per-peer RSA-ключ, см. ниже).
...
`sub` = `peer_pub` (стабильная идентичность). Остальное — из `block.profile`
(см. [IDENTITY.md](IDENTITY.md)), с fallback на синтез, пока профиля нет:

- `profile` → `name` (first/last), `preferred_username` = `profile.name`
  (имя пира). Fallback: имя пира из роестра.
- `email` → `profile.email` = `<name>@<zone>.f2f`, плюс `email_verified: false`
  (это идентификатор, а не доставляемый адрес). Fallback: `<fp8>@<zone>.f2f`.
...
- **Камп-вайд репликация** passkey-credential'ов (вход удалённого гостя у
  чужого IdP) — через `block.profile` (scope `profiles`), см. [IDENTITY.md](IDENTITY.md).
