...
- `profile` → `name` = полное имя из `block.profile` (first+last), fallback —
  имя пира из роестра; `preferred_username` = **имя пира** (стабильный handle
  без пробелов), не полное имя.
- `email` → `<fp8>@<zone>.f2f`, `email_verified: false` — синтетический
  стабильный идентификатор (не из профиля: email там нет, а fp8 не меняется при
  ренейме и не конфликтует у приложений, матчащих юзера по email).
