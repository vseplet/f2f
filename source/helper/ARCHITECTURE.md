...
⏳ **#6 `engine/awg/device.go`** — создание `device.NewDevice` на Start
   ...
   `listen_port=0` — потому что bind свой, `Jc/H1..H4` derive'нутые
   из camp_id). Без peer'ов пока.

⏳ **#7 Peer sync** — на каждый успешный verify pair-handshake +
   camp poll генерить блок peer'ов в IpcSet (`public_key + endpoint
...
⏳ **#9 Tunnel routing вынос** — `tun.Read`/`tun.Write` больше не
...
⏳ **#10 Hardening: drop plaintext IPv4/IPv6 на UDP-сокете** —
   ...
   без аутентификации" (см. секцию "Status model" выше — после этого
   шага плoperации со старыми нодами становятся невозможны, они
   автоматически 🔴 unreachable).
...
После #7 шифрование работает. После #9 plaintext остаётся **только**
как fallback к не-обновлённым пирам (если флаг включён). После #10
plaintext запрещён полностью — единственный способ говорить с нами
по UDP это pair envelope или AWG envelope.

> **Шаг "Pinger move into tunnel"** из первой редакции плана
> **выпилен** — pair_req/pair_res поглотили RTT-измерение целиком,
> отдельный pinger больше не существует. Commit `7757470` удалил
> `engine/peerping/` пакет.
