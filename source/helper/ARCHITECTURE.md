...
1. **Camp не трогаем вообще**. Camp-сервер остаётся как есть, ноль
   ...
   между пирами** через расширенный hole-punch (см. "Hello-handshake"
...
3. **Существующие фичи engine не теряем**: NAT-traversal (теперь
   через hello, а не 1-байт `0x00`), camp announce, pinger,
...
#### Обмен WGPub через hello-handshake (НЕ через camp)
...
расширенный hole-punch.

##### Hello-packet

Сейчас hole-punch — это 1 байт `0x00`. **Hello полностью заменяет
этот пакет**: тот же `holePunchLoop`, та же роль (открывать NAT-мэппинг
встречными UDP-пакетами), но теперь несёт identity + WGPub + подпись.
Один тип пакета, две функции — больше нет двух разных control-сигналов
на сокете.
...
где
```
sig = ed25519.Sign(ed_priv, "f2f-hello-v1|" + name + "|" + pub + "|" + wg_pub)
```

Доменный тэг `f2f-hello-v1` защищает от replay в других контекстах
(invite-токены, будущие подписанные сообщения) — одна Ed25519-подпись
никогда не может быть применена за пределами своего домена.

Этот JSON **не идёт в plaintext** — он завёрнут в obfuscation-envelope
(см. ниже), и multiplex'ом разруливается по custom magic header'у, а
не по первому байту `{`.

##### Verify flow

При получении hello-пакета:

1. Найти peer по `pub` (Ed25519) в `e.peers`. Ed25519 pub'у мы доверяем
   ровно настолько, насколько доверяем camp'у — это знакомая точка
   доверия, та же что сейчас (sticky-binding по pub'у).
2. Если peer'а с такой `pub` нет — drop. Не-член camp'а пытается
   подключиться.
3. Если есть — проверить подпись:
   `ed25519.Verify(peer.Pub, "f2f-hello-v1|...", sig)`.
4. Подпись валидна → атомарно сохранить `peer.WGPub = wg_pub`.
5. Если `peer.WGPub` уже был и новый значение отличается — log
   warning (не должно быть в нормальной жизни; возможный сигнал
   о компрометации или о намеренной ротации ключей peer'ом).

##### Lifecycle

Hello шлётся **на каждой итерации holePunchLoop** — раз в 1Hz пока
peer не верифицирован, раз в ~25с keepalive после установления
связи. Это значит:

- **Identity-attestation continuous, не разовая** — атакующий, который
  влез после первоначального соединения, должен подделывать
  Ed25519-подпись на каждом keepalive (криптографически невозможно
  без `priv.key`).
- Потеря отдельного hello не критична — следующая итерация принесёт
  его заново.
- Изменение Ed25519 identity peer'ом (rotated `priv.key`) → hello с
  новой `pub` → не находится в `e.peers` → drop'ается. Корректное
  поведение: ротация identity требует обновления camp roster через
  announce.

##### Что НЕ кладём в hello

- `overlay_v4` — derive'ится из `pub` детерминистически
  (`overlay.PubToV4Addr`), передавать не нужно.
- `domains`, `files`, `intercepts` — это polling-данные, меняются в
  runtime, остаются в `/api/*` endpoint'ах поверх tunnel.
- `udp_endpoint` — сам факт получения hello даёт `from` адрес.
...
##### Обфускация: control-envelope для hello

Hello в plaintext-виде — это JSON с `pub`, `wg_pub`, `sig` — для DPI
очень узнаваемая сигнатура: те же поля каждый punch tick, та же
структура, hex-строки фиксированной длины. Если DPI настроится
конкретно на f2f, hello-пакеты будут палиться мгновенно.

Поэтому hello заворачивается в **control-envelope** того же духа,
что AWG-обёртка трафика:

```
[magic_h5..h8 (4 bytes, random in negotiated range)]
[nonce (12 bytes, random per packet)]
[ChaCha20-Poly1305(camp_key, json_hello_body)]
...
- Поэтому атакующий с invite видит наши hello, но **атакующий без
...
| Header | Что внутри |
|---|---|
| `H5` | hello (то что описано выше) |
| `H6` | резерв (будущие control-типы — например, AWG-profile-handoff, key-rotation request) |
| `H7` | резерв |
| `H8` | резерв |

Все восемь диапазонов генерятся при создании camp'а, кладутся в
`<camp_id>.config.json`, распространяются через invite-токен. Они
**не пересекаются** между собой и не пересекаются со старыми
...
##### Pinger переезжает внутрь WG-туннеля

Сейчас pinger (`engine/peerping/`) шлёт JSON `{"t":"ping",...}` на
raw UDP-сокете для измерения RTT и проверки двусторонней
достижимости.

После внедрения WG: **pinger пишет ping в utun, читает pong из utun**.
Пакеты идут как обычные IP-пакеты между overlay-адресами
`100.64.X.Y`, шифруются AWG, выглядят на проводе как любая другая
data. DPI ничего про pinger не видит — он внутри `H1..H4` AWG-обёртки.

Когда pinger остаётся на raw UDP:

- **Plaintext-fallback пиры** (один на старой версии без AWG):
  pinger продолжает работать через raw UDP. Чтобы не светить JSON
  паттерн — заворачиваем pinger ping/pong в тот же control-envelope
  (`H5..H8`), отдельный `"t": "ping"` тип. Это закрывает последнюю
  раздачу plaintext'а в peer-to-peer трафике.
- **Перед AWG handshake'ом** (только что установили соединение,
  ещё не handshake'нулись): тоже raw UDP в envelope'е. Окно
  короткое — handshake завершается за ~1с.

После handshake'а — pinger живёт исключительно через tunnel.
...
Сейчас (до AWG) `peerToTunLoop` (`engine.go:3335`) дискриминирует:
- `from == campAddr` + JSON → camp announce reply
- pinger ping/pong (JSON `{"t":"ping",...}` — первый байт `{`)
- 1-байт hole-punch (`0x00`)
- IPv4/IPv6 пакет → `tun.Write`

После внедрения AWG + control-envelope (см. секции "Hello-handshake"
и "Обфускация" выше):

| Первый uint32 | Что | Дальше |
|---|---|---|
| `H1..H4` | AWG transport/handshake | `awgBind.Recv` → Device расшифровывает → пишет в utun |
| `H5..H8` | control-envelope (hello, ping в fallback) | decrypt camp_key → JSON → диспатч по `"t"` |
| `0x40..0x4F` | IPv4 packet | `tun.Write` (только для plaintext-fallback пиров) |
| `0x60..0x6F` | IPv6 packet | `tun.Write` (только для plaintext-fallback) |
| `{` (`0x7B`) | camp announce reply | как сейчас (camp пока plaintext, см. open question) |
| прочее | drop |  |

**1-байт hole-punch ИСЧЕЗАЕТ** — его роль (открывать NAT) полностью
поглощена hello'ом в envelope'е.
...
| 1-байт hole-punch (`0x00`) | **Исчезает** — hello заменяет полностью |
| Hello (JSON, signed) в control-envelope | **Появляется** — открывает NAT + identity + WGPub, обфусцирован |
| Pinger (raw UDP) | **Двойной режим**: через AWG-tunnel после handshake'а (как обычный IP-пакет); через control-envelope в plaintext-fallback и в окне до handshake'а |
| Camp announce, peer poll | **Остаётся в engine** (camp по-прежнему plaintext, см. open question) |
| Мультиплекс на recv | **Добавляется** (новая логика в `peerToTunLoop`) |
...
| `peer.WGPub` (из verified hello) | `public_key=<hex>` |
...
3. **Передаётся через hello-handshake** между пирами (как WGPub).
...
### Hello, NAT и AWG handshake — порядок

Hello и hole-punch — это **один и тот же пакет** в новом дизайне.
Hello open'ит NAT (полезный UDP-пакет с peer-to-peer endpoint'а на
peer-to-peer endpoint), и заодно несёт identity + WGPub в обёртке
control-envelope. Никаких отдельных 1-байт пакетов. Порядок:

1. **Camp poll** — engine узнаёт, что у peer X появился endpoint
   `1.2.3.4:5678` и его Ed25519 `pub`. WGPub camp **не знает**.
2. **Engine hello (через holePunchLoop)** — шлёт hello-пакет в
   control-envelope (`H5` magic + ChaCha20-Poly1305) на endpoint.
   Это и есть hole-punch — пакет одновременно открывает NAT и
   несёт подписанный identity + свой WGPub. Если peer X тоже шлёт
   встречный hello — NAT с обеих сторон открыт, мы получили его WGPub.
3. **Engine verify hello** — расшифровывает envelope (camp_key),
   парсит JSON, проверяет ed25519-подпись против `pub` peer'а
   (известного из camp poll'а). Невалидная подпись → drop, peer
   остаётся в plaintext fallback.
4. **Engine кормит AWG endpoint'ом + WGPub'ом** —
   `device.IpcSet("public_key=<verified_wg_pub>\nendpoint=1.2.3.4:5678\nallowed_ip=...")`.
   Если hello ещё не получен или невалиден — IpcSet не вызывается.
5. **AWG keepalive** — Device отправляет первый handshake init через
   уже открытый NAT (наш hello его пробил, идёт по той же траектории).
6. **Handshake завершается** — обмен симметричными ключами, AllowedIPs
   активны, трафик через этого peer'а шифруется.
7. **Pinger переключается в tunnel** — после успешного handshake'а
   pinger пишет ping в utun (на overlay-адрес peer'а), AWG шифрует.
   RTT измеряется уже внутри зашифрованного канала.

Важно: **AWG keepalive ≠ engine hello**. AWG keepalive шлёт
зашифрованные WG-transport-пакеты раз в N секунд для поддержания
WG session state'а (`H1..H4` magic); hello — это control-envelope
пакет (`H5` magic) с подписанным identity'ом. Они уживаются
параллельно: hello продолжает continuous attestation + open NAT,
AWG keepalive поддерживает WG state.

### Backward compatibility

| Сценарий | Поведение |
|---|---|
| Оба пира — новая версия, обмен hello успешен и подпись валидна | Шифрованный transport через AmneziaWG |
| Один пир на старой версии (шлёт `0x00` hole-punch вместо hello) | Plaintext fallback с этим peer'ом (новый ждёт hello, не получает) |
| Hello получен, но подпись невалидна (camp подменил? битый пакет?) | Drop hello, plaintext fallback, warning в логи |
| Hello валиден, но AWG handshake не прошёл (network / отказ peer'а) | Logging warning, plaintext fallback по timeout'у |
...
от которых не пришёл валидный hello + WGPub. Это уже политическое
...
1. **`identity.X25519()`** — derive X25519 keypair из Ed25519 seed
   через HKDF. Unit-тест на детерминизм (одна seed → один и тот же
   x25519). Pub публикуется методом `X25519PubHex()`.
2. **Control-envelope (`engine/obfenv/`)** — `Seal`/`Open` функции
   обёртки ChaCha20-Poly1305 + magic header. `camp_key` derive'ится
   из `camp_id` через HKDF (`info="f2f-control-v1"`). Конфиг magic
   header диапазонов `H5..H8` хранится в `<camp_id>.config.json`
   рядом с AWG `H1..H4`. На этом шаге envelope готов, но никем
   ещё не используется. Unit-тесты на roundtrip + reject невалидного
   tag'а.
3. **Hello-handshake протокол** — shape JSON-пакета, canonical
   message для подписи (`f2f-hello-v1|name|pub|wg_pub`), sign/verify
   в `identity`. Hello шлётся из `holePunchLoop` обёрнутый в
   envelope из шага #2 (`H5` magic). **Полностью заменяет 1-байт
   `0x00` hole-punch.** `peerToTunLoop` учится распознавать `H5`,
   расшифровывать, верифицировать подпись, класть `WGPub` в
   `peer.WGPub`. **Camp не трогается вообще.**
4. **`engine/awg/bind.go`** — реализация `conn.Bind` поверх `e.udp`.
   `Send`/`Receive`/`ParseEndpoint`/`BatchSize`. Device пока не
   создаётся, bind просто существует.
5. **`engine/awg/discriminator.go`** — расширяем мультиплекс в
   `peerToTunLoop`: AWG-пакеты (`H1..H4`) отдаём в bind. AWG-пакеты
   пока дропаются (Device нет).
6. **`engine/awg/device.go`** — создание `device.NewDevice` на Start
   в camp mode. IpcSet с base config (`private_key`,
   `listen_port=0` — потому что bind свой, `Jc/H1..H4` из camp
   config'а). Без peer'ов пока.
7. **Peer sync** — на каждый успешный verify hello + camp poll
   генерить блок peer'ов в IpcSet (`public_key + endpoint +
   allowed_ip + persistent_keepalive`). После этого шага шифрование
   работает между обновлёнными участниками.
8. **Intercept sync** — `addIntercept`/`removeIntercept` пишут
   `allowed_ip=<prefix>` через UAPI peer'а. Старые intercept'ы из
   `<camp_id>.config.json` мигрируются на Start.
9. **Pinger move into tunnel** — pinger пишет ping в utun на
   overlay-адрес peer'а вместо raw UDP. После handshake'а ping/pong
   идут через AWG-шифрование автоматически. Для plaintext-fallback
   пиров (нет AWG) pinger остаётся на raw UDP, но в control-envelope
   (`H7` тип `"t":"ping"`). Закрывает последнюю plaintext-раздачу
   в peer-to-peer.
10. **Tunnel routing вынос** — `tun.Read`/`tun.Write` больше не
    вызываются из engine, Device читает/пишет в utun сам.
    `tunToPeerLoop` удаляется, `peerToTunLoop` сжимается до
    мультиплекса. Логика `routeFor` и `interceptPeerForLocked`
    удаляются — `Device.allowedips` их заменяет.
11. **Plaintext fallback policy** — флаг `RequireAWG` в camp config,
    UI-toggle, warning'и в логах при handshake failure.

После #7 шифрование работает. После #9 единственный peer-to-peer
plaintext-канал — это IPv4/IPv6 для plaintext-fallback пиров (старые
версии). После #10 plaintext остаётся **только** как fallback к
не-обновлённым.
...
5. **Существующие intercept'ы при первом запуске AWG**. Сейчас они
   живут в `<camp_id>.config.json`, восстанавливаются в
   `e.intercepts` на Start. После #8 нужно автоматом перенести их
   в `allowed_ips` соответствующих peer'ов через IpcSet. Без миграции
   — UI покажет intercept существующим, но трафик не пойдёт.
...
8. **`camp_key` rotation при компрометации**. `camp_key` derive'ится из
   ...
   расшифровывать наш control-envelope (видеть hello'ы, но не
   подделывать — подпись держит). Ротация требует смены camp_id, что
