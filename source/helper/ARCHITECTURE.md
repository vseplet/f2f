...
> **Статус (2026-06): сделано.** Весь peer↔peer-трафик ушёл на QUIC-bus
> (`mesh/bus`): domains, ca-cert, firewall, files (poll-сервисы) и
> signal, call.state/join/leave/signal (real-time) — каждый сервис
> регистрирует свой bus-тип через Register(). HTTP-over-tunnel и
> tunnel-listener (BindTunnel) удалены целиком; на туннеле наружу
> остались только 2203/udp (bus), 80/443 (reverse-proxy доменов) и
> 6881 (торрент) — порт 2202 убран из builtin firewall и из engine
> (TunnelHTTPPort больше нет). bus чистит коннекты к ушедшим из roster
> пирам (evictStale) и не пингует офлайновых.
>
> **Что НЕ сделано:** push-модель. Сервисы по-прежнему опрашивают пиров
> по тикеру, просто поверх QUIC, а не HTTP. Замена poll → push (слать
> обновления только при изменении) — следующий шаг, см. FIXES.md.
...
| Builtin firewall ports | `services/firewall` | `2203,80,443,6881` |
...
> **Статус (2026-06): ядро OIDC-провайдера реализовано** — см.
> [`OIDC.md`](OIDC.md) и `services/oidc/`. Дизайн ниже — исходный план;
> реализация разошлась с ним в нескольких местах:
> - **Не центральный `auth.xyz.f2f`, а per-peer** `auth-<fp8>.<zone>.f2f`
>   (+ co-located `/oidc` на доменах приложений): зона общая на камп, один
>   лейбл не может быть IdP каждого пира.
> - **Generic OIDC, без мимикрии** под PocketID/Authelia: приложения едят
>   стандартный discovery. Реальная проблема совместимости была не «формат
>   провайдера», а **алгоритм подписи** → перешли на **RS256** (EdDSA
>   клиенты вроде Gitea не принимают).
> - **Фаза 1 без `/api/sign`-агента**: транспорт (AmneziaWG) уже
>   аутентифицирует пира, прокси инжектит `X-F2F-Peer` → `sub`. Это и есть
>   «фаза 2 zero-click» — она доступна уже сейчас. Passkey-церемония идёт
>   прямо на `/authorize` (WebAuthn, `rpId=<zone>.f2f`).
>
> Ещё **не сделано** из плана ниже: SSO-сессия, per-service ACL, группы и
> политики, защита самого портала, forward-auth для сервисов без OIDC.
