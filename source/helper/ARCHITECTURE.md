...
| Что | Зачем |
|---|---|
| `engine.go` | orchestrator: lifecycle (Start/Stop), peers map (`peerState`), OnStarted/OnStopped callbacks для main.go |
| `config_store.go` | hydrate peers из catalog в memory map, mergePeerSnapshotLocked под camp poll |
...
| `rendezvous/` | camp client: UDP announce + HTTP peer-list poll, `PeerInfo` wire-формат |
...
| Пакет | Что |
|---|---|
| `config.Store` | thread-safe wrapper над `~/.f2f/<camp_id>.config.json`. `Snapshot(id)` отдаёт deep-copy, `UpdateCamp(id, fn)` атомарно читает→мутирует→пишет. In-memory cache синхронизирован с диском. Источник правды для всех сервисов. |
| `identity` | derive Ed25519 keypair из persistent seed, fingerprint = первые 8 байт SHA-256(pub). `X25519()` через HKDF — для AWG transport keys. `CampLabel(id)` — короткий display-label из camp_id. |
