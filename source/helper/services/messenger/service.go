...
// a message this one quotes ("" for none).
func (s *Service) SendDM(peerPub, body string, file *Attachment, replyTo string) (Message, error) {
	selfPub := s.self()
	if selfPub == "" {
	...
	m := Message{
		...
		From: selfPub, To: peerPub, Body: body, File: file, ReplyTo: replyTo, TS: nowMs(),
	}
	local := m
	local.Mine = true
...
// message this one quotes ("" for none).
func (s *Service) Post(chanID, body string, file *Attachment, replyTo string) (Message, error) {
	return s.emit(chanID, TypeText, body, nil, file, replyTo)
}
...
// used.
func (s *Service) emit(chanID, typ, body string, roster []string, file *Attachment, replyTo string) (Message, error) {
	// general has no stored channel/roster — fan out to the whole camp.
	if isGeneral(chanID) {
		peers := s.generalRoster()
		return s.emitTo(chanID, typ, body, peers, nil, peers, file, replyTo)
	}
	s.mu.Lock()
	ch := s.channels[chanID]
	s.mu.Unlock()
	...
	return s.emitTo(chanID, typ, body, roster, nil, roster, file, replyTo)
}
...
func (s *Service) emitTo(chanID, typ, body string, roster, affected, fanout []string, file *Attachment, replyTo string) (Message, error) {
	selfPub := s.self()
	m := Message{
		ID: newID(), Kind: "channel", Peer: chanID, Type: typ,
		From: selfPub, Body: body, File: file, ReplyTo: replyTo,
