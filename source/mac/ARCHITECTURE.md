...
14. [Пакет `internal/web` — HTTP UI + reverse-proxy](#internalweb)
15. [Главные сущности (типы)](#главные-сущности)
16. [Сценарии: куда идёт каждый пакет](#сценарии-куда-идёт-каждый-пакет)
17. [Дизайнерские решения и почему так](#дизайнерские-решения)
...
Что делает при `Open(iface, allowed []PortRule)`:

1. Кладёт в anchor `com.apple/f2f-mac-fw` ruleset вида:
   ```
   block in on utun7 all                      # default deny — ВАЖНО что первый
   pass out on utun7 keep state               # outbound не ограничен
   pass in on utun7 proto icmp keep state     # ping всегда работает
   pass in on utun7 proto tcp ... port { 2202 80 443 6881 22 } keep state
   pass in on utun7 proto udp ... port { 6881 } keep state
   ```
2. Сохраняет state (PID, anchor, iface, список разрешённых правил)
   JSON-ом в `/var/run/f2f-mac.firewall.json`. На следующем запуске
   читаем этот файл (`sweepLeftover`) и если процесс с PID-ом мёртв
   — flush'аем anchor за него.
...
**Порядок правил критичен.** pf использует last-matching-rule
семантику (без `quick`): если `block in` стоит **после** `pass`-
правил, он перекрывает их и всё блокирует. Поэтому `block` всегда
**первый**, `pass`-правила **после** — и для входящего пакета на
port 443 матчится сначала block, потом pass, последний (pass)
побеждает.
