...
18. [Дизайнерские решения и почему так](#дизайнерские-решения)
...
Весь WebRTC-трафик идёт через tunnel overlay (100.64.0.0/10), т.е.
ICE использует только host candidates на tunnel IP. STUN/TURN не нужен.
...
type SFU struct {
    api          *webrtc.API
    participants map[string]*Participant  // keyed by tunnel_ip
...
1. **Хост создаёт звонок** → `engine.CreateCall()` → `sfu.New()` → `AddParticipant(hostIP)`.
2. **Удалённый пир join'ит** → POST на `/api/call/join` хоста → `AddParticipant(peerIP)`.
   При добавлении: существующие треки других участников добавляются в новый PC,
   запускается `forwardRTCP` для каждой пары трек↔подписчик, через 1с шлётся PLI
   для быстрого первого keyframe.
...
4. **OnTrack** — SFU получает трек от издателя → создаёт `TrackLocalStaticRTP` клон →
   добавляет его во все остальные PC → вызывает `renegotiate()` (шлёт SFU-offer подписчикам).
5. **RTP forwarding** — горутина: `remote.Read(buf)` → `local.Write(buf[:n])`.
   Без транскодинга, пакеты пробрасываются как есть.
6. **RTCP forwarding** — для каждого `RTPSender` на подписчике запускается горутина
   `forwardRTCP`: читает RTCP (PLI/FIR) от подписчика и форвардит на PC издателя.
   Это восстанавливает нативную цепочку обратной связи, которую SFU разрывает.
7. **Chat** — DataChannel relay: каждый участник создаёт DC `chat`, SFU бродкастит
   сообщения всем остальным через `SendText`.
8. **Leave** — хост уходит → `SFU.Close()` закрывает все PC. Удалённый пир уходит →
   `RemoveParticipant()` удаляет треки из других PC + рененогоциация.

### Сигнализация

SFU-сигналы (offer/answer/candidate) доставляются через HTTP:

- **Хост ↔ свой браузер**: браузер POST на `/api/call/signal` (loopback) →
  engine обрабатывает → ответ в POST response. SFU-initiated offers идут через
  `deliverSFUSignal` → POST на tunnel listener → `handleCallSignalInbound` →
  проверка `from: "sfu"` → broadcast в `callSignals` SSE → браузер.
- **Удалённый пир ↔ SFU**: браузер POST на `/api/call/signal` (loopback) →
  ...
  SFU-initiated offers идут обратным путём через tunnel → SSE.

### Call discovery

`callPollLoop` (3с) опрашивает `/api/call/state` у всех онлайн-пиров.
Найденные звонки сохраняются в `engine.remoteCalls`. UI показывает список
всех звонков (локальных + удалённых) через `GET /api/call/list`.
...
| GET | `/api/call/signal/stream` | loopback | SSE для SFU-initiated сигналов |
