...
type state struct {
	...
	TunnelV6 string     `json:"tunnel_v6,omitempty"`
...
// cleaned up first.
func Open(iface, tunnelIP, tunnelV6 string, allowed []PortRule) (*Firewall, error) {
	if iface == "" {
	...
	if err := pfLoadAnchor(anchorName, buildRules(iface, tunnelIP, tunnelV6, f.state.Allowed)); err != nil {
...
func (f *Firewall) Apply(allowed []PortRule) error {
	allowed = dedup(allowed)
	if err := pfLoadAnchor(f.state.Anchor, buildRules(f.state.Iface, f.state.TunnelIP, f.state.TunnelV6, allowed)); err != nil {
...
// fully permitted.
func buildRules(iface, tunnelIP, tunnelV6 string, allowed []PortRule) string {
	...
	emitHost := func(family, host, mask, icmpProto string) {
		fmt.Fprintf(&sb, "block in on %s from any to %s/%s\n", iface, host, mask)
		// ICMP — diagnostics; surface is essentially zero.
		fmt.Fprintf(&sb, "pass in on %s proto %s from any to %s/%s keep state\n", iface, icmpProto, host, mask)
		if len(tcp) > 0 {
			fmt.Fprintf(&sb, "pass in on %s proto tcp from any to %s/%s port { %s } keep state\n",
				iface, host, mask, joinInts(tcp))
		}
		if len(udp) > 0 {
			fmt.Fprintf(&sb, "pass in on %s proto udp from any to %s/%s port { %s } keep state\n",
				iface, host, mask, joinInts(udp))
		}
		_ = family
	}
	emitHost("inet", tunnelIP, "32", "icmp")
	if tunnelV6 != "" {
		emitHost("inet6", tunnelV6, "128", "icmp6")
	}
	return sb.String()
}
