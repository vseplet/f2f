...
- `f2f-mac ui [--bind 127.0.0.1:8080]` — поднимает локальную веб-морду в браузере. Тот же движок, что и `run`, только управляется через HTTP.
...
- добавлять/удалять интерсепты на лету (без рестарта движка);
- видеть счётчики tx/rx пакетов и байт, текущий peer-адрес (с авто-обновлением);
- читать живой лог через SSE.

По умолчанию UI слушает только на `127.0.0.1` — никто из локалки не достучится. Если очень нужно выставить наружу: `--bind 0.0.0.0:8080`, но **аутентификации пока нет** — делай это осознанно.
...
Python-тесты сами собирают свежий бинарь и стартуют `f2f-mac ui` на свободном порту — никаких внешних предусловий.
...
7. Fake-IP пул + split DNS (чтобы браузер хочешь не хочешь шёл через туннель)
8. IPv6-стэк на utun + IPv6 NAT (сейчас IPv6 утекает мимо туннеля)
9. UDP hole punching через rendezvous-сервер
10. Стыковка с виндовой половиной
