...
- **Inbound firewall** на utun — default-deny через `pf` anchor. F2F-внутренние
  порты (`2202/tcp`, `80/tcp`, `443/tcp`, `6881/tcp+udp`) всегда открыты,
  остальные доступны только если пользователь явно открыл в Tunnel-табе.
  Любой сервис который слушает на `0.0.0.0` (sshd, postgres, dev-серверы)
  по умолчанию **не достижим** через `<tunnel_ip>`.
...
9. Поднимает **inbound firewall** на утане: `pf` anchor `com.apple/f2f-mac-fw` с `block in on utunN all` + `pass`-правилами для built-in портов (`2202/tcp`, `80/tcp`, `443/tcp`, `6881/tcp+udp`) и любых пользовательских из `~/Library/Application Support/f2f/firewall.json`. Сохраняет state в `/var/run/f2f-mac.firewall.json` для отката.
10. **BitTorrent client** (anacrolix): биндит `<tunnel_ip>:6881` (TCP), без DHT/PEX/публичных tracker'ов, с `Seed = true` и `DisableIPv6 = true`. Раздача из `~/Library/Application Support/f2f/shared/`, скачивание в `~/Downloads/f2f-drops/`.
11. Запускает воркеры:
