...
9. **BitTorrent client** (anacrolix): биндит `<tunnel_ip>:6881` (TCP), без DHT/PEX/публичных tracker'ов, с `Seed = true` и `DisableIPv6 = true`. Раздача из `~/Library/Application Support/f2f/shared/`, скачивание в `~/Downloads/f2f-drops/`.
10. Запускает воркеры:
