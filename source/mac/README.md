...
9. Поднимает **inbound firewall** на утане: `pf` anchor `com.apple/f2f-mac-fw` с `block in on utunN to <our_tunnel_ip>/32 all` + `pass`-правилами для built-in портов (`2202/tcp`, `80/tcp`, `443/tcp`, `6881/tcp+udp`) и любых пользовательских из `~/Library/Application Support/f2f/firewall.json`. Scope'нут на наш tunnel_ip — egress-форвардинг и трафик к другим peer'ам не трогает. Сохраняет state в `/var/run/f2f-mac.firewall.json` для отката.
