...
- **Per-camp identity** — на каждый camp_id движок генерит свой
  Ed25519 keypair (`/var/lib/f2f/identity/<camp_id>/{priv,pub}.key`).
  Публичный ключ зеркалится в `<camp_id>.config.json`. Пока ключи
  только лежат — в будущем по ним будем подписывать announce и
  invite-токены.
- **Per-camp config** — все user-editable настройки (alias в camp-е,
  intercepts, my-domains, firewall, trusted-peer fingerprints, последний
  снапшот peer-каталога) лежат в `$HOME/.f2f/<camp_id>.config.json`.
  Глобальный `$HOME/.f2f/state.json` хранит `last_camp_id` и список
  известных camp'ов для UI-дропдауна.
- **Autostart** — при запуске бинаря движок сам поднимается на
  `last_camp_id` из `state.json`, UI становится доступен сразу.
...
**Когда движок остановлен** — единственный контрол: дропдаун со списком
известных camp'ов плюс пункт `+ new camp` снизу. Выбрал известный →
движок сам стартует (имя берётся из `<camp_id>.config.json`). Выбрал
`+ new camp` — раскрывается форма `name + camp id`, жмёшь `join`.

**Когда движок работает** — вместо формы показывается key:value
readout: твой `name`, `camp id`, `tunnel ip`, наблюдаемый camp-сервером
`endpoint` (public_ip:port), полный `public key` (кликаемый — копирует
hex в буфер) + короткий fingerprint. Ниже `switch camp →` — сбрасывает
к дропдауну, выбираешь другой camp, движок сам делает stop+start.
...
1. Читает `$HOME/.f2f/<camp_id>.config.json` (создаёт если нет). Оттуда же поднимает intercepts, my-domains, firewall, trusted-peer fingerprints. Апсертит `state.json` (`last_camp_id`, `known_camps`).
2. Генерит/загружает Ed25519 keypair под `/var/lib/f2f/identity/<camp_id>/` (приватник 0600, паблик 0644). Pub-key + fingerprint мирорятся в `<camp_id>.config.json`.
3. Открывает utun, ставит на него адрес из camp-а (`10.99.0.X`).
...
10. Включает egress NAT: `pf` anchor `com.apple/f2f-mac` с правилом `nat on en0 from 10.99.0.0/24 to any -> (en0)`, плюс `sysctl net.inet.ip.forwarding=1`. Старое значение forwarding сохраняется в `/var/run/f2f-mac.egress.json` для отката.
...
| `$HOME/.f2f/<camp_id>.config.json` | per-camp: alias, intercepts, my-domains, firewall, trusted-peer fingerprints, peer catalog, identity pub | юзер |
...
| POST | `/api/start` | старт (тело: `{camp_id, camp_name?}`; авто-stop+start если crash-switch) |
| POST | `/api/stop` | остановка |
| GET | `/api/camps` | глобальный state.json — `last_camp_id` + `known_camps[]` |
| GET | `/api/camp/{id}` | полный `<id>.config.json` |
